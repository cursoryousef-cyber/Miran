    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      <div>
        <h1 style={{ fontSize: '24px', fontWeight: 800, color: '#f8fafc', margin: 0 }}>
          خدمة التقارير والتحليلات المستقلة (Decoupled Reporting Service)
        </h1>
      ...
      {/* Available Templates */}
      <div>
        <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#f8fafc', marginBottom: '16px' }}>قوالب التقارير المتاحة</h3>
        <TableContainer component={Paper} className="glass-card">
          <Table>
            <TableHead>
              <TableRow>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>اسم التقرير</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>النوع</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>الصيغة الافتراضية</TableCell>
            ...
            <TableBody>
              {definitions?.map((def: any) => (
                <TableRow key={def.id}>
                  <TableCell style={{ fontWeight: 700, color: '#f8fafc' }}>{def.nameAr}</TableCell>
      ...
      <div>
        <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#f8fafc', marginBottom: '16px' }}>سجل التقارير المُنتجة الجاهزة للتحميل</h3>
        <TableContainer component={Paper} className="glass-card">
          <Table>
            <TableHead>
              <TableRow>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>اسم التقرير</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>تاريخ التوليد</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>عدد السجلات</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>الحالة</TableCell>
                <TableCell style={{ color: '#94a3b8', fontWeight: 700 }}>تحميل</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {myReports?.map((rep: any) => (
                <TableRow key={rep.id}>
                  <TableCell style={{ fontWeight: 700, color: '#f8fafc' }}>{rep.reportDefinition?.nameAr}</TableCell>
                  <TableCell>{new Date(rep.createdAt).toLocaleString('ar-SA')}</TableCell>
                  <TableCell style={{ fontWeight: 700, color: '#34d399' }}>{rep.rowCount} سجل</TableCell>
